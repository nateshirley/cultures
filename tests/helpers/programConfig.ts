...
export interface NewSmartCollectionConfig {
  mint: PublicKey;
  mintPair: Keypair;
  smartCollection: Pda;
...
export const getCultureConfig = async (name: string) => {
  let culture = await addresses.findCulture(name);
  let cultureInfo = await Cultures.account.culture.fetch(culture.address);
  return {
    culture: culture,
    name: name,
    creatorMint: cultureInfo.creatorMint,
    audienceMint: cultureInfo.audienceMint,
    ...
    stakePatrol: await addresses.findPatrol("stake_patrol", culture.address),
  };
};

export const getNewCultureConfig = async (
  name: string,
  creatorMint: PublicKey,
  audienceMint: PublicKey
...
export const getNewSmartCollectionConfig = async (
  culture: Pda
): Promise<NewSmartCollectionConfig> => {
  let mint = web3.Keypair.generate();
  let collectionPatrol = await addresses.findPatrol(
    "collection_patrol",
    culture.address
  );
  return {
